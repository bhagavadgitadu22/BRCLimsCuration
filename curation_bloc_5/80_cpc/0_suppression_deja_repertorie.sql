DROP TABLE IF EXISTS suppressions_deja_repertories;
...
OR TRIM(REPLACE(sch_identifiant, 'CIP', '')) = tsv.svl_valeur
...
OR REPLACE(sch_references_equi, ' ', '') LIKE CONCAT('%', REPLACE(tsv.svl_valeur, ' ', ''), '%')
OR REPLACE(tsvta.svl_valeur, ' ', '') LIKE CONCAT('%', REPLACE(REPLACE(tsv.svl_valeur, ' Enterococcus', ''), ' ', ''), '%')
OR REPLACE(tsvta.svl_valeur, ' ', '') LIKE CONCAT('%', REPLACE(REPLACE(REPLACE(tsv.svl_valeur, ' Enterococcus', ''), ' ', ''), '_', ''), '%'))

ORDER BY sch_denomination;

UPDATE t_string_val
SET svl_valeur = ''
FROM suppressions_deja_repertories
WHERE svl_entite_id = suppressions_deja_repertories.xxx_id
AND svl_att_id IN (SELECT xxx_id
FROM t_attribut 
WHERE att_nom = 'Code produit collection'
