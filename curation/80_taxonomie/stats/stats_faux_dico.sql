...
WHERE (level = 0 AND name_path[1] NOT IN (SELECT genus_name FROM taxonomy WHERE genus_name IS NOT NULL))
	OR (level = 1 AND (name_path[1], name_path[2]) NOT IN (SELECT genus_name, sp_epithet FROM taxonomy WHERE sp_epithet IS NOT NULL))
	OR (level = 2 AND (name_path[1], name_path[2], name_path[3]) NOT IN (SELECT genus_name, sp_epithet, subsp_epithet FROM taxonomy WHERE subsp_epithet IS NOT NULL))
	OR level > 2
