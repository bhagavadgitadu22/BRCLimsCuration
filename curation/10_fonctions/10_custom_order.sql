...
$$
	BEGIN
		...
		ELSIF trim($1) SIMILAR TO 'CIP [0-9]{2}.[0-9]+' THEN
			RETURN 2000000+(REGEXP_MATCHES($1, '([0-9]{2}).[0-9]+'))[1]::decimal*10000+(REGEXP_MATCHES($1, '[0-9]{2}.([0-9]+)'))[1]::decimal;
		ELSIF trim($1) SIMILAR TO 'CIP [0-9]{2}.[0-9]+T' THEN
			RETURN 2000000+(REGEXP_MATCHES($1, '([0-9]{2}).[0-9]+'))[1]::decimal*10000+(REGEXP_MATCHES($1, '[0-9]{2}.([0-9]+)'))[1]::decimal+0.5;
