...
DROP TABLE IF EXISTS emplacements_libres_stockeur_2;
...
ON ids_balancelles_2.xxx_id = lts_lst_id
GROUP BY cst_tst_id;
...
INTO emplacements_libres_stockeur_2
FROM
(SELECT ids_balancelles_2.xxx_id AS id_balancelle, lst_nom, 
t_casestockage.xxx_id AS id_case, cst_numero, cst_pos_x, cst_pos_y
FROM ids_balancelles_2, t_casestockage
WHERE cst_tst_id IN (SELECT * FROM id_boite_180)
EXCEPT
SELECT ids_balancelles_2.xxx_id, lst_nom, 
t_casestockage.xxx_id, cst_numero, cst_pos_x, cst_pos_y
FROM t_casestockage
JOIN t_lot_casestockage
ON t_casestockage.xxx_id = lts_cst_id
JOIN ids_balancelles_2
ON ids_balancelles_2.xxx_id = lts_lst_id) AS a
