...
DROP TABLE IF EXISTS ids_post_nulls_a_changer;
...
WITH RECURSIVE children (xxx_id, don_lib, level, don_code, name_path) AS (
	...
	WHERE
		don_dic_id = 3755
		AND (don_lib != 'NULL' OR (don_lib = 'NULL' AND don_parent = 0))
	UNION
		(SELECT
		...
		WHERE tdd.don_dic_id = 3755 AND tdd.don_lib = 'NULL')
...
WITH RECURSIVE children (xxx_id, don_lib, level, don_code, name_path) AS (
	...
	FROM
		t_donneedico
	WHERE
		xxx_id IN (SELECT sch_taxonomie FROM ids_nulls_a_changer)
	UNION
		(SELECT
			tdd.xxx_id, tdd.don_lib, t0.level + 1, tdd.don_code, ARRAY_APPEND(t0.name_path, tdd.xxx_id)
		FROM
...
) SELECT DISTINCT xxx_id AS sch_taxonomie, ids_nulls_a_changer.new_sch_taxonomie
INTO TEMPORARY TABLE ids_post_nulls_a_changer 
FROM children
JOIN ids_nulls_a_changer 
ON ids_nulls_a_changer.sch_taxonomie = name_path[1]
...
FROM ids_post_nulls_a_changer
WHERE t_souche.sch_taxonomie = ids_post_nulls_a_changer.sch_taxonomie
AND t_souche.sch_taxonomie != ids_post_nulls_a_changer.new_sch_taxonomie;
...
WHERE xxx_id IN (SELECT sch_taxonomie FROM ids_post_nulls_a_changer);
