...
WITH RECURSIVE children (xxx_id, don_lib, level, don_code, name_path) AS (
	...
	FROM
		t_donneedico
	WHERE
		xxx_id IN (SELECT sch_taxonomie FROM ids_empty_a_changer)
	UNION
		(SELECT
			tdd.xxx_id, tdd.don_lib, t0.level + 1, tdd.don_code, ARRAY_APPEND(t0.name_path, tdd.xxx_id)
		FROM
...
) SELECT DISTINCT xxx_id AS sch_taxonomie, ids_empty_a_changer.new_sch_taxonomie
INTO TEMPORARY TABLE ids_post_empty_a_changer 
FROM children
JOIN ids_empty_a_changer 
ON ids_empty_a_changer.sch_taxonomie = name_path[1]
...
WHERE t_souche.sch_taxonomie = ids_post_empty_a_changer.sch_taxonomie
AND t_souche.sch_taxonomie != ids_post_empty_a_changer.new_sch_taxonomie;
...
WHERE xxx_id IN (SELECT sch_taxonomie FROM ids_post_empty_a_changer);
