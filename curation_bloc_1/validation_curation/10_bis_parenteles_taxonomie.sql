...
WITH RECURSIVE children (xxx_id, don_lib, level, don_code, name_path, don_dic_id, num_path) AS (
	SELECT
		xxx_id, don_lib, 0, don_code, ARRAY[don_lib::text], don_dic_id, ARRAY[don_code::text]
	FROM
		t_donneedico
	...
	UNION
		(SELECT
			tdd.xxx_id, tdd.don_lib, t0.level + 1, tdd.don_code, ARRAY_APPEND(t0.name_path, tdd.don_lib::text), tdd.don_dic_id, ARRAY_APPEND(t0.num_path, tdd.don_code::text)
		FROM
			t_donneedico tdd
...
ARRAY_TO_STRING(num_path, ' > ') AS path2
