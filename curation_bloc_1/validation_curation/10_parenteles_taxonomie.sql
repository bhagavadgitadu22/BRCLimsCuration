...
WITH RECURSIVE children (xxx_id, don_lib, level, don_code, name_path, don_dic_id, xxx_sup_dat) AS (
	SELECT
		xxx_id, don_lib, 0, don_code, ARRAY[don_lib::text], don_dic_id, xxx_sup_dat
	FROM
		t_donneedico
	...
	UNION
		(SELECT
			tdd.xxx_id, tdd.don_lib, t0.level + 1, tdd.don_code, ARRAY_APPEND(t0.name_path, tdd.don_lib::text), tdd.don_dic_id, tdd.xxx_sup_dat
		FROM
			t_donneedico tdd
...
(regexp_matches(dic_grp_collection, '\[([0-9]+)\]'))[1]::integer AS grp_collection, children.xxx_sup_dat
