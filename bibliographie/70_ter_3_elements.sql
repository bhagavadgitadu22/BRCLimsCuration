SELECT journal, doc, annee, volume, pages, array_agg(sch_identifiant) FROM

(SELECT full_trim(doc[1]) AS journal,
 doc[3],
 full_trim(doc[2]) AS annee,
 (REGEXP_MATCHES(full_trim(doc[2]), '^([0-9]{4}).*?[0-9]+$'))[1] AS volume,
 (REGEXP_MATCHES(full_trim(doc[2]), '^[0-9]{4}.*?([0-9]+)$'))[1] AS pages,
 sch_identifiant
FROM 
...
GROUP BY journal, doc, annee, volume, pages;
