...
# we keep the global rows (the one about CARE and the one about ResFinder) and we transpose them
df = df.T
for name in df.columns:
    if name != 'All genera combined' and name != 'Total ResFinder':
        df = df.drop(name, 1)
print(df)

# we organize the plot with a log scale
plt.rcParams["figure.figsize"] = (20,8)
ax = df.plot(kind="barh", width = 0.8)
wrap_labels(ax, 30)
plt.title("Number of resistance genes per antimicrobial class in CARE vs in ResFinder")
...
plt.savefig('C:/Users/mboutrou/Documents/global_results.png')
