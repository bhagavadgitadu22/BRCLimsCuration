...
def get_all_souches(c):
    c.execute(open("../curation_bloc_5/validation_curation/0_toutes_souches.sql", "r").read())
    records = c.fetchall()
...
def main():
    ...
    fa = open('../../output/bloc_5/souches_supprimees.csv', 'w', newline='')
    ...
    cursor.execute(open("../curation_bloc_5/validation_curation/10_parenteles_taxonomie.sql", "r").read())
    cursor_curated.execute(open("../curation_bloc_5/validation_curation/10_parenteles_taxonomie.sql", "r").read())
    ...
    for sch in souches_a_garder:
        ...
        if record != record_curated:
            if record[len(record)-18] == 401 and record_curated[len(record_curated)-18] == 401:
                if record[7] is None:
    ...
    f = open('../../output/bloc_5/cip_modifies.csv', 'w', newline='')
