...
/* taxos pas utilisees du tout et fausses a priori */
SELECT ids_ok.sch_taxonomie, don_lib, path, array_to_string(ARRAY_AGG(t_souche.sch_identifiant), ', '), COUNT(*)
FROM ids_ok
LEFT JOIN t_souche
...
/* taxos utilisees et fausses a priori */
SELECT ids_ok.sch_taxonomie, don_lib, path, array_to_string(ARRAY_AGG(t_souche.sch_identifiant), ', '), COUNT(*)
FROM ids_ok
LEFT JOIN t_souche
ON ids_ok.sch_taxonomie = t_souche.sch_taxonomie
