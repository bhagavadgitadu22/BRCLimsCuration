...
UPDATE t_souche
SET sch_temps_culture = REGEXP_REPLACE(sch_temps_culture, '(?<=[0-9]+)( | -| -  ?|- |à|, | ?/ ?| ?H ?-? ?|_| D-?){1}(?=[0-9]+)', '-')
WHERE xxx_id IN (SELECT xxx_id FROM souches_groupe_cip)
AND sch_temps_culture != REGEXP_REPLACE(sch_temps_culture, '(?<=[0-9]+)( | -| -  ?|- |à|, | ?/ ?| ?H ?-? ?|_| D-?){1}(?=[0-9]+)', '-');
...
UPDATE t_souche
SET sch_temps_culture = REGEXP_REPLACE(sch_temps_culture, '(?<=[0-9]+)[ ]*(?=[a-zA-Z]+$)', ' ')
WHERE xxx_id IN (SELECT xxx_id FROM souches_groupe_cip)
AND sch_temps_culture != REGEXP_REPLACE(sch_temps_culture, '(?<=[0-9]+)[ ]*(?=[a-zA-Z]+$)', ' ');
