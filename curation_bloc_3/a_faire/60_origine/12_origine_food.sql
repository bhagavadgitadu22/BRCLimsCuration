...
SELECT xxx_id, sch_isole_a_partir_de, btrim(REGEXP_REPLACE(sch_isole_a_partir_de, '(F|f)ood', ''), ', ()*') AS new_isole
...
/*
SELECT (SELECT xxx_id FROM nombre_food), CONCAT(UPPER(LEFT(new_isole, 1)), RIGHT(new_isole, -1))
FROM t_souche
JOIN origine_food
