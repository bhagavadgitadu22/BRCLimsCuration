...
SELECT xxx_id, sch_isole_a_partir_de, btrim(REGEXP_REPLACE(sch_isole_a_partir_de, '(A|a)nimal', ''), ', ()*') AS new_isole
...
/*
SELECT (SELECT xxx_id FROM nombre_animal), CONCAT(UPPER(LEFT(new_isole, 1)), RIGHT(new_isole, -1))
FROM t_souche
JOIN origine_animal
