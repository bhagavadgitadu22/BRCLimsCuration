...
SELECT xxx_id, sch_isole_a_partir_de, btrim(REGEXP_REPLACE(sch_isole_a_partir_de, '(H|h)uman(?=([^A-Za-z]+|$))', ''), ', *:.-') AS new_isole
...
SELECT DISTINCT t_donneedico.xxx_id, 'Human' AS category
...
/*
SELECT (SELECT category FROM nombre_humain), CONCAT(UPPER(LEFT(new_isole, 1)), RIGHT(new_isole, -1))
FROM t_souche
